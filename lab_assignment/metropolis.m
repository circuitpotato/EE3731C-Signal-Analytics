function [accept_new_key, p_accept] = metropolis(current_key, new_key, pr_trans, encrypted_txt)
...
current_encrypt_double = char2double(encrypted_txt);
current_decrypt_double = current_key(current_encrypt_double);
current_decrypt_char = double2char(current_decrypt_double);

current_log = logn_pr_txt(current_decrypt_char, pr_trans);

new_encrypt_double = char2double(encrypted_txt);
new_decrypt_double = new_key(new_encrypt_double);
new_decrypt_char = double2char(new_decrypt_double);

log_new = logn_pr_txt(new_decrypt_char, pr_trans);

if (log_new >= current_log)
    accept_new_key = 1;
    p_accept = 1;
else
    p_accept = exp(log_new - current_log);
    if (rand(1) < p_accept)
        accept_new_key = 1;
    else
        accept_new_key = 0;
